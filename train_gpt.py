...
class Hyperparameters:
    ...
    # CEP: Covariant Embedding Perturbation
    # Persistent spectral geometry transformation on embedding space.
    # Modes: "off" (disabled), "perturbation" (1+ε*σ), "direct" (σ^ε)
    cep_mode = os.environ.get("CEP_MODE", "off")
    cep_eps = float(os.environ.get("CEP_EPS", "0.1"))

# --- Bigram co-occurrence SVD (for CEP) ---

def compute_bigram_svd(data_path: str, vocab_size: int, model_dim: int):
    """Compute SVD of bigram co-occurrence matrix from training data.
    Returns (U_M, S_M) or (None, None) if no data found.
    """
    shard_files = sorted(Path(data_path).glob("fineweb_train_*.bin"))
    if not shard_files:
        shard_files = sorted(Path(data_path).glob("fineweb_val_*.bin"))
    if not shard_files:
        return None, None
    ...
    for sf in shard_files[:1]:
        ...
        tokens = np.fromfile(sf, dtype="<u2", count=num_tokens, offset=header_bytes)
        tokens = tokens[tokens < vocab_size]
        for i in range(len(tokens) - 1):
            M[tokens[i], tokens[i + 1]] += 1
    U_M, S_M, _ = np.linalg.svd(M, full_matrices=False)
    return (
        U_M[:, :model_dim].astype(np.float32),
        S_M[:model_dim].astype(np.float32),
    )


class CovariantEmbeddingPerturbation(nn.Module):
    """Learnable metric tensor on embedding space derived from bigram SVD.
    Rotate into co-occurrence basis, apply per-direction scaling, rotate back.
    Adds d learnable parameters (~2KB). Basis recomputed from data at load time.
    """
    def __init__(self, model_dim: int, U_M: np.ndarray, S_M: np.ndarray,
                 eps: float = 0.1, mode: str = "perturbation"):
        super().__init__()
        d = model_dim
        basis_raw = U_M[:d, :d].copy()
        Q, _ = np.linalg.qr(basis_raw)
        self.register_buffer("basis", torch.from_numpy(Q.astype(np.float32)), persistent=False)
        sigma_norm = S_M[:d].copy()
        sigma_norm = sigma_norm / (sigma_norm[0] + 1e-10)
        if mode == "perturbation":
            init_scale = 1.0 + eps * sigma_norm
        elif mode == "direct":
            init_scale = np.power(np.clip(sigma_norm, 1e-6, None), eps)
        else:
            init_scale = np.ones(d, dtype=np.float32)
        self.scale = nn.Parameter(torch.from_numpy(init_scale.astype(np.float32)))

    def forward(self, x: Tensor) -> Tensor:
        x_rot = x @ self.basis
        x_scaled = x_rot * self.scale
        return x_scaled @ self.basis.T
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,smear,dtg_gate,ve_layer_scales,ve_shared.scale,attn_gate,vr_lambda,cep.scale",
    ).split(",")
    if pattern
)
...
class GPT(nn.Module):
    def __init__(
        ...
        cep_mode: str = "off",
        cep_eps: float = 0.1,
        cep_U_M: np.ndarray | None = None,
        cep_S_M: np.ndarray | None = None,
    ):
        super().__init__()
        ...
        # CEP: Covariant Embedding Perturbation
        if cep_mode != "off" and cep_U_M is not None and cep_S_M is not None:
            self.cep = CovariantEmbeddingPerturbation(model_dim, cep_U_M, cep_S_M, eps=cep_eps, mode=cep_mode)
        else:
            self.cep = None
...
def main() -> None:
    ...
    # CEP: Compute bigram SVD if needed
    cep_U_M, cep_S_M = None, None
    if args.cep_mode != "off":
        log0(f"cep:computing bigram SVD for mode={args.cep_mode} eps={args.cep_eps}")
        t_svd = time.perf_counter()
        cep_U_M, cep_S_M = compute_bigram_svd(args.data_path, args.vocab_size, args.model_dim)
        if cep_U_M is not None:
            log0(f"cep:bigram SVD completed in {time.perf_counter() - t_svd:.2f}s "
                 f"U_shape={cep_U_M.shape} S_range=[{cep_S_M[0]:.1f}, {cep_S_M[-1]:.4f}]")
        else:
            log0("cep:WARNING no data found, disabling CEP")
            args.cep_mode = "off"
    else:
        log0("cep:disabled")
    ...
    base_model = GPT(
        ...
        cep_mode=args.cep_mode,
        cep_eps=args.cep_eps,
        cep_U_M=cep_U_M,
        cep_S_M=cep_S_M,
    ).to(device).bfloat16()
    ...
    eval_model = GPT(
        ...
        cep_mode=args.cep_mode, cep_eps=args.cep_eps,
        cep_U_M=cep_U_M, cep_S_M=cep_S_M,
    ).to(device).bfloat16()
