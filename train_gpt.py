...
def compute_bigram_svd(data_path: str, vocab_size: int, model_dim: int):
    """Compute SVD of bigram co-occurrence matrix. Returns (U_M, S_M) or (None, None)."""
    ...
    return U_M[:, :model_dim].astype(np.float32), S_M[:model_dim].astype(np.float32)
...
def main() -> None:
    ...
    if args.cep_mode != "off":
        ...
        if cep_U_M is not None:
            log0(f"cep:SVD done in {time.perf_counter() - t_svd:.2f}s U={cep_U_M.shape} S=[{cep_S_M[0]:.1f},{cep_S_M[-1]:.4f}]")
        else:
            log0("cep:WARNING no data, disabling CEP")
            args.cep_mode = "off"
    else:
        log0("cep:disabled")
